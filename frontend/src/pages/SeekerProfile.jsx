...
import { User, MapPin, Settings, Edit, ArrowRight, X } from 'lucide-react';
...
const SeekerProfile = () => {
  ...
  const isOwnProfile = !userId || userId === user?.id || userId === user?.userId;
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        setProfileData({
          name: dbData.name || '',
          email: dbData.email || '',
          title: dbData.currentRole || '',
          location: dbData.location || '',
          role: dbData.role || '',
          skills: dbData.skills || [],
          bio: dbData.bio || '',
          profilePicture: dbData.avatarUrl || null
        });
      } catch (error) {
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      // ✅ Update AuthContext + localStorage so navbar name updates immediately
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {profileData.skills.map((skill, index) => (
                      <span key={index} className="px-3 py-1 rounded-full text-xs font-medium bg-secondary-100 text-secondary-700">
                        {skill}
                      </span>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={handleCancelEdit} className="text-secondary-400 hover:text-secondary-600">
                <X size={24} />
              </button>
            </div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="w-20 h-20 rounded-full bg-secondary-100 flex items-center justify-center">
                    {tempProfileData.profilePicture ? (
                      <img src={getImageUrl(tempProfileData.profilePicture)} alt="Profile" className="w-full h-full rounded-full object-cover" />
                    ) : (
